...
def tokenize_lines(input):
    ...
    input sequence."""
    return (tokenize_line(line) for line in input) # line是表示一行输入的字符串，这里将其映射为该输入行的token list。

def count_tokens(input):
    """Count the number of non-delimiter tokens in input."""
    return len(list(itertools.chain(*tokenize_lines(input)))) # *对实参list拆包。

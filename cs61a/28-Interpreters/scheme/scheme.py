...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if val != None:
            return val
        if self.parent != None:
            return self.parent.lookup(symbol) # 递归向上查找。
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        frame = Frame(self)
        fp, vp = formals, vals
        while fp is not nil and vp is not nil:
            frame.define(fp.first, vp.first)
            fp = fp.rest
            vp = vp.rest
        if fp is not nil or vp is not nil:
            raise SchemeError("formals and vals mismatching")
        return frame
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            # If calling the function results in a TypeError exception being raised,
            # then the wrong number of arguments were passed.
            raise SchemeError(e)
        # END PROBLEM 3
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals, args) # 注意这是在父frame下创建子frame。
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        check_form(expressions, 2, 2) # 检查expressions这个list的长度恰好是2，即一个符号和一个值。
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        # Pair(A, Pair(B, nil)), where: A is the symbol being bound, B is an expression whose value should be bound to A.
        # (eval (define tau 6.28)) ? 6.28
        # 注意要对要绑定到的表达式求值，如果是`(define g (lambda (x) (* x x)))`，那么符号g绑定到的表达式就是一个lambda过程对象，
        return env.define(target, scheme_eval(expressions.rest.first, env))
        # END PROBLEM 5
        ...
        # (define (f x) (* x 2))
        # 不对形参列表和body求值。
        return env.define(target.first, do_lambda_form(Pair(target.rest, expressions.rest), env))
        # END PROBLEM 9
    else:
...
def do_begin_form(expressions, env):
    ...
    check_form(expressions, 1) # expr list的长度最小为1。
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # (lambda (x y) (+ x y))，该输入会被解释器存储为list，第一个pair的car指向符号lambda，第二个pair的car指向list (x y)，等。
    # 因为过程体可能有多条expr，即多个list，所以这里传入expressions.rest，而不是expressions.rest.first。
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    # (not (and #t #f 42 (/ 1 0))) ? #t 该表达式不会抛出异常，这就是短路求值。
    if expressions is nil:
        # return '#t'
        return True
    val = scheme_eval(expressions.first, env)
    # 在scheme中只有#f才表示False，其它都表示#t，包括0。
    # if val == False:
...
def do_cond_form(expressions, env):
    ...
    # (cond ((p, e), ...)), 其中传入给expressions的是((p, e), ...)
    # 下面遍历(p, e)。
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest is nil: # 没有e，返回p的求值结果。
                return test
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    formals, vals = Pair(-1, nil), Pair(-1, nil) # 哨兵。
    f, v = formals, vals
    while bindings is not nil:
        binding = bindings.first
        check_form(binding, 2, 2) # (a 1 1)是不行的，(a (+ 1 2))就可以。
        f.rest = Pair(binding.first, nil)
        v.rest = Pair(scheme_eval(binding.rest.first, env), nil)
        bindings = bindings.rest
        f = f.rest
        v = v.rest
    check_formals(formals.rest)
    return env.make_child_frame(formals.rest, vals.rest) # 记得去掉哨兵。
    # END PROBLEM 14
...
def do_quasiquote_form(expressions, env):
    ...
    def quasiquote_item(val, env, level):
        ...
        if val.first == 'unquote':
            ...
            level += 1 # 只有最底层的那个unquote的参数才会被求值。
...
# This means that when a MuProcedure created by a mu expression is called,
# the parent of the new call frame is the environment in which the call expression was evaluated.
...
class MuProcedure(Procedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        return env.make_child_frame(self.formals, args) # 注意这是在传入的env下创建frame。
    # END PROBLEM 15
...
SPECIAL_FORMS['mu'] = do_mu_form # 把新的special form加入表格中。

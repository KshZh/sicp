...
class Literal(Expr):
    ...
    def eval(self, env):
        return Number(self.value) # 对Literal求值得到Number。
...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        "*** YOUR CODE HERE ***"
        try:
            return env[self.string] # 对变量Name求值得到变量在求值环境中关联的Value。
        except KeyError:
...
class LambdaExpr(Expr):
    ...
    def eval(self, env):
        return LambdaFunction(self.parameters, self.body, env) # 对LambdaExpr求值得到一个LambdaFunction，可被调用。
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        # 在上面的测试中，如果没有拷贝全局环境到局部环境，然后在局部环境中求值，而是直接在全局环境中求值的话，
        # 那么add变量会从关联primitive procedure变为关联Number(8)，导致调用失败。
        childEnv = self.parent.copy() # 深拷贝。这里我们没有一个向上查找变量的步骤，简单粗暴地将父frame拷贝到当前frame中。
        for k, v in zip(self.parameters, arguments): # 在当前frame中绑定形参变量与实参值。
            childEnv[k] = v
        return self.body.eval(childEnv) # 在当前环境中进行求值。
...
class PrimitiveFunction(Value):
    ...
    def apply(self, arguments):
        ...
        # *[]是拆包。
        return Number(self.operator(*[arg.value for arg in arguments])) # apply callable的结果是一个Number，当然如果允许高阶函数的话，结果还可以是一个callable。

...
def scheme_read(src):
    ...
    val = src.pop()
    if val == 'nil':
        return nil # nil是一个单例对象，是nil类的实例，表示一个空list。
    elif val not in DELIMITERS:  # ( ) ' .
        return val # 返回23、True、'+'、'abc'等。
    elif val == "(":
...
def buffer_input():
    return Buffer(tokenize_lines(InputReader('> '))) # 将list of token lists放入Buffer，Buffer就像一个流，可以逐步消耗其中的token。

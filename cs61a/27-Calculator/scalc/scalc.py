...
def calc_eval(exp):
    ...
    if type(exp) in (int, float):
        return simplify(exp)
    elif isinstance(exp, Pair): # 表达式是用连起来的Pair，即list来表示的。
        arguments = exp.second.map(calc_eval) # 对后面的每一个元素，即参数应用calc_eval。
...
def reduce(fn, scheme_list, start):
    ...
    6
    """
    if scheme_list is nil: # is是比较是否是同一个对象。
        return start
    return reduce(fn, scheme_list.second, fn(start, scheme_list.first)) # 注意这里递归调用的是这里定义的reduce，而不是python的reduce。

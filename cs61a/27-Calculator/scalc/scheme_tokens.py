...
_NUMERAL_STARTS = set(string.digits) | set('+-.') # 以.开头的浮点数。
_WHITESPACE = set(' \t\n\r')
...
def next_candidate_token(line, k):
    ...
    while k < len(line):
        ...
        if c == ';':
            ...
            return line[k:k+2], min(k+2, len(line)) # 注意这里min的使用，防止溢出。
        else:
            # 一个多字符的token。
            j = k
...
def tokenize_lines(input):
    ...
    return map(tokenize_line, input) # 对每一行应用tokenize_line，即将每一个表示行的字符串映射为一个token list。
